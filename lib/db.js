...
// Store RAW token, not hashed
export const storeShop = (shop, accessToken, scopes) => {
  try {
    if (!shop || !accessToken || !scopes) throw new Error('Missing required parameter for storing shop');
...
export const getAccessToken = (shop) => {
  try {
    ...
    return row ? row.access_token : null; // raw token
  } catch (e) {
...
export const storeSitemap = (shop, url, data) => {
  try {
    ...
    db.prepare(`INSERT INTO sitemaps (shop, url, data) VALUES (?, ?, ?)`).run(shop, url, JSON.stringify(data));
    return true;
  } catch (e) {
...
export const getSitemaps = (shop) => {
  const rows = db.prepare(`SELECT * FROM sitemaps WHERE shop = ? ORDER BY created_at DESC`).all(shop);
