...
export async function POST(req) {
  ...
  if (!verifyWebhookHmac(rawBody, hmac, process.env.SHOPIFY_API_SECRET)) {
    console.error("Invalid HMAC for customers/data_request webhook from shop:", shopDomain);
  ...
  try {
    ...
    console.error("Failed to parse customers/data_request payload from shop:", shopDomain, err);
    return new Response(JSON.stringify({ error: "Bad Request" }), { status: 400 });
  }

  // ----------------- Step 3: Handle the request -----------------
  // Free app: no customer data stored, so this is a no-op
  // Shopify expects 200 OK to acknowledge receipt
  console.log(`Received customers/data_request from shop: ${shopDomain}`);
