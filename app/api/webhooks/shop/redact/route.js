...
export async function POST(req) {
  ...
  if (!verifyWebhookHmac(rawBody, hmac, process.env.SHOPIFY_API_SECRET)) {
    console.error("Invalid HMAC for shop/redact webhook from shop:", shopDomain);
  ...
  try {
    ...
    console.error("Failed to parse shop/redact payload from shop:", shopDomain, err);
    return new Response(JSON.stringify({ error: "Bad Request" }), { status: 400 });
  }

  // ----------------- Step 3: Handle the request -----------------
  // Free app: no shop data stored outside tokens and basic metadata
  // Shopify expects 200 OK to acknowledge the webhook
  console.log(`Received shop/redact webhook from shop: ${shopDomain}`);
  console.log("Request ID:", payload.id || "N/A");

  // If you stored any shop-specific data, delete it here. For now, it's a no-op.
